...
class GUI:
    def __init__(self, num_frames):
        layout = [
            [
                Sg.Column([[
                    Sg.Image(filename='', key='_IMAGEL_', pad=(0, 0))],
                    [Sg.Slider(range=(1, num_frames), orientation='h', size=(37, 20), pad=(0, 5), default_value=0,
                               enable_events=True,
                               key='_SLIDERL_'), Sg.Button('Pause', focus=True), Sg.Button('Quit')],
                ], ),
                Sg.Column([[
                    Sg.Image(filename='', key='_IMAGER_', pad=(0, 0))],
                    [Sg.Slider(range=(1, num_frames), orientation='h', size=(37, 20), pad=(0, 5), default_value=0,
                               enable_events=True,
                               key='_SLIDERR_'), Sg.Button('Pause', focus=True), Sg.Button('Quit')],
                ])

            ]
        ]

        self.window = Sg.Window('Real-time', location=(0, 0), margins=(0, 0), element_padding=(0,0), layout=layout, resizable=True,
                                element_justification="center").Finalize()
        self.window.Maximize()

    def update_frame(self, img):
        encoded_img = cv.imencode('.png', cv.resize(img, (self.window, 650)))[1].tobytes()
        self.window['_IMAGE_'].update(data=encoded_img)

    def update_frame_L_and_R(self, img_l, img_r):
        encoded_img_l = cv.imencode('.png', cv.resize(img_l,
                                                      (int(self.window.size[0] / 2), self.window.size[1] - 60)))[1].tobytes()
        encoded_img_r = cv.imencode('.png', cv.resize(img_r,
                                                      (int(self.window.size[0] / 2), self.window.size[1] - 60)))[1].tobytes()
        self.window['_IMAGEL_'].update(data=encoded_img_l)
        self.window['_IMAGER_'].update(data=encoded_img_r)

    def update_slider(self, pos):
        self.window['_SLIDERL_'].update(pos)
        self.window['_SLIDERR_'].update(pos)
...
class App:
    ...
    def run(self):
        ...
        while self.cam.isOpened():
            ...
            self.curr_left = left_frame
            self.curr_right = right_frame
            ...
            if self.tooltip:
                ...
                cv.addWeighted(copy, circle_opacity, vis_left, 1 - circle_opacity, 0, vis_left)

                copy = vis_right.copy()
                cv.circle(copy, (self.tooltip_right[0], self.tooltip_right[1]), rect_size, circle_color, thickness=2)
                cv.addWeighted(copy, circle_opacity, vis_right, 1 - circle_opacity, 0, vis_right)

                # out = np.hstack((vis_left, vis_right))

                if outcome == App.DISPARITY_MAP_ERROR_CODE:
                    if distance is None:
                        self.draw_str(vis_left, (20, 20), 'DISP MAP QUALITY ISSUE')
                        self.draw_str(vis_right, (20, 20), 'DISP MAP QUALITY ISSUE')
                    else:
                        self.draw_str(vis_left, (20, 20), 'WARNING')
                        self.draw_str(vis_right, (20, 20), 'WARNING')
                elif outcome == App.UNSAFE_ERROR_CODE:
                    self.draw_str(vis_left, (480, 50), 'DISTANCE WARNING', font_size=3.0, color=(0, 0, 255))
                    self.draw_str(vis_right, (480, 50), 'DISTANCE WARNING', font_size=3.0, color=(0, 0, 255))
            ...
            if _PLAY:
                if int(gui_values['_SLIDERL_']) != self.frame_idx - 1:
                    self.frame_idx = int(gui_values['_SLIDERL_'])
                    self.cam.set(cv.CAP_PROP_POS_FRAMES, self.frame_idx)

                # increment frame index
                self.frame_idx += 1
                if out is not None:
                    # self.GUI.update_frame(out)
                    self.GUI.update_frame_L_and_R(vis_left, vis_right)
    ...
    def draw_str(dst, target, s, font_size=1.0, color=(255, 255, 255)):
        x, y = target
        cv.putText(dst, s, (x + 1, y + 1), cv.FONT_HERSHEY_PLAIN, font_size, (0, 0, 0), thickness=3,
                   lineType=cv.LINE_AA)
    ...
    def depth_estimation(self, left, right, centroid):
        ...
        disparity = cv.normalize(cv.resize(disparity, (0, 0), fx=1 / .6, fy=1 / .6), None, 0, 255, cv.NORM_MINMAX,
                                 cv.CV_8U)
    ...
    def detect_tip(self):
        # resize image
        curr_img = cv.resize(self.curr_left, (320, 240))
        ...
        # resize image
        curr_img = cv.resize(self.curr_right, (320, 240))
        # expand dims to include batch size
        _input = np.expand_dims(curr_img, axis=0)
        # run CNN
        pred = self.loaded_model.predict(_input / 255.0, batch_size=1)
        ...
        pred[0][1] = pred[0][1] * 320
        pred = pred.reshape(2)
        self.tooltip_right = tuple(np.rint(pred * 4.275).astype(int))
